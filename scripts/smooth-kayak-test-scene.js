...
export function setup(gumInstance, assets) {
	...
	// Audio Locations
	cidada_location = randomWorldPoint(200, 200)
...
export function draw(delta) {
	...
	let sighDiffMag = .5 / Math.abs(sigh_diff.x) + .5 / Math.abs(sigh_diff.z)
...
async function paddle(direction) {
	return new Promise((resolve, reject) => {
		...
		fatigue_tooltip.innerText = exertion_feelings[Math.floor(Math.random() * exertion_feelings.length)];
	})
}
